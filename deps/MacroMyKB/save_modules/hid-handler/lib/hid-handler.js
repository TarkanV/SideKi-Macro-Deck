...
that = _.extend(new events.EventEmitter(), {
  ...
  handleEndpointData: function (hid, ifaceIndex, inEndpointIndex, data) {
    ...
    eventClass = hid.device.eventClass[ifaceIndex];
    event = new eventClass(hid, ifaceIndex, inEndpointIndex, data);
  ...
  registerHid: function (opt, cb) {
    ...
    return p.do(
      ...
      function initHid() {
        ...
        hid.device.eventClass = {};
        hid.device.keyLayoutHandler = {};
        _.forIn(hid.device.type, function (type, key) {
          var eventClassName;
          if (typeof type === 'string') {
            (function (typeParts) {
              ...
              if (typeParts.length > 1) {
                type.layout = typeParts[1];
              }
              if (typeParts.length > 2) {
                type.variant = typeParts[2];
              }
            })(type.split('-'));
          }
          hid.device.type[key] = type;
          eventClassName = util.format('%sEvent', _.capitalize(_.camelCase(type.value)));
          hid.device.eventClass[key] = that[eventClassName] || that.GenericEvent;
        });
      },
      function openDevice() {
  ...
  start: function (opt, cb) {
    ...
    return p.do(
      ...
      function initKeyLayouts() {
        return keyLayouts.init(that.config.keyLayouts);
      },
      // ##### REPLACE the entire original registerDevices function with this one #####

      function registerDevices() {
        var supportedDevices;
        var atLeastOneDeviceFound = false; // Our new flag
        ...
        // This promise chain will now correctly reject on failure
        return p.each(_.values(supportedDevices),
          function (supportedDevice) {
            ...
            atLeastOneDeviceFound = true; // We found one!
            deviceType = supportedDevice.type || that.config.defaultDeviceType;
          ...
          .then(function() {
            // This .then() block runs AFTER the loop is fully complete.
            if (!atLeastOneDeviceFound) {
              // If we never found a device, we throw an error to reject the main promise.
              throw new Error("No matching USB devices found. Will retry.");
            }
          });
      })
...
(function exportsEventEmitter() {
  var eventEmittersKeys = Object.keys(events.EventEmitter.prototype);
  eventEmittersKeys.forEach(function (key) {
    if (typeof that[key] === 'function') {
      logger.enabledLevels.trace && log.trace('exports event emitter function "%s"', key);
      exports[key] = that[key].bind(that);
    }
  });
})();

(function exportsFuncs(moduleFuncs) {
  moduleFuncs.forEach(function (key) {
    logger.enabledLevels.trace && log.trace('exports "%s"', key);
    exports[key] = that[key].bind(that);
  });
})([
  'getSupportedDevice',
