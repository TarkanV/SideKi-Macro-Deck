...
class winsock {
    ...
    Recv(flags:=0) {
        ...
        buf := Buffer(bytes,0)
        bytesReceived := DllCall("Ws2_32\recv","UInt",this.desc,"UPtr",buf.ptr,"Int",buf.size,"Int",flags)
         If (bytesReceived < 0) {
        ...
        ; Return a buffer with only the received data
        if (bytesReceived < bytes) {
            result := Buffer(bytesReceived, 0)
            DllCall("RtlMoveMemory", "UPtr", result.ptr, "UPtr", buf.ptr, "UPtr", bytesReceived)
            return result
        }
